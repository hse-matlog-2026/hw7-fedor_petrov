...
class Term:
    ...
    def _parse_prefix(string: str) -> Tuple[Term, str]:
        ...
        arguments = []
        while True:
            argument, suffix = Term._parse_prefix(suffix)
            arguments.append(argument)
            if suffix[0] == ')':
                return Term(name, arguments), suffix[1:]
            assert suffix[0] == ','
            suffix = suffix[1:]
    ...
    def parse(string: str) -> Term:
        ...
        """
        # Task 7.3b
        term, suffix = Term._parse_prefix(string)
        assert suffix == ''
        return term
...
class Formula:
    ...
    def _parse_prefix(string: str) -> Tuple[Formula, str]:
        ...
        if is_unary(first_char):
            ...
            return Formula(first_char, formula), suffix
        ...
        if first_char == '(':
            ...
            assert suffix[0] == ')'
            return Formula(root, first, second), suffix[1:]

        if is_quantifier(first_char):
            name_end = 2
            while name_end < len(string) and string[name_end].isalnum():
                name_end += 1
            variable = string[1:name_end]
            assert is_variable(variable)
            assert string[name_end] == '['
            statement, suffix = Formula._parse_prefix(string[name_end + 1:])
            assert suffix[0] == ']'
            return Formula(first_char, variable, statement), suffix[1:]

        if is_relation(first_char):
            name_end = 1
            while name_end < len(string) and string[name_end].isalnum():
                name_end += 1
            relation = string[:name_end]
            assert is_relation(relation)
            assert string[name_end] == '('
            suffix = string[name_end + 1:]
            arguments = []
            if suffix[0] == ')':
                return Formula(relation, arguments), suffix[1:]
            while True:
                argument, suffix = Term._parse_prefix(suffix)
                arguments.append(argument)
                if suffix[0] == ')':
        ...
        first, suffix = Term._parse_prefix(string)
        assert suffix[0] == '='
        second, suffix = Term._parse_prefix(suffix[1:])
        return Formula('=', [first, second]), suffix
    ...
    def free_variables(self) -> Set[str]:
        ...
        if is_relation(self.root) or is_equality(self.root):
            free_variables = set()
            for argument in self.arguments:
                free_variables.update(argument.variables())
